...
export default function AdminProjectTagsPage() {
  ...
  const openEditModal = (tag: ProjectTag) => {
    setEditingTag(tag);
    setTagId(tag.id);
    setTagName(tag.name);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingTag ? `/api/admin/project-tags/${editingTag.id}` : '/api/admin/project-tags';
  ...
  const handleDeleteConfirm = async () => {
    ...
    const res = await fetch(`/api/admin/project-tags/${deleteTarget.id}`, { method: 'DELETE' });
    const data = await res.json();
    if (!res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          <h1 className="font-heading text-3xl font-bold text-white uppercase">
            Quản Lý Project Tags
          </h1>
          <p className="text-xs font-mono text-[#a3a3a3] mt-1">
            Danh sách các tag phân loại cho FL Studio Projects
          </p>
        </div>
        ...
        >
          ...
          <span>Thêm Project Tag Mới</span>
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-[#111111] rounded-2xl border border-white/5">
          <p className="text-sm text-white font-mono">Chưa có project tag nào</p>
        </div>
      )}
